...
export default function JobFamilyPage() {
  ...
  return (
    <div>
      <h1 className='text-2xl font-bold text-gray-800 mb-1'>Job Family</h1>
      <p className='text-gray-500 text-sm mb-6'>{t('Pengelompokan jabatan berdasarkan fungsi atau bidang keahlian.','Job grouping based on function or area of expertise.')}</p>

      <div className='grid grid-cols-1 lg:grid-cols-3 gap-6'>
        {/* Form */}
        <div className='bg-white rounded-xl p-6 shadow-sm'>
          <h2 className='text-sm font-bold text-gray-700 mb-4'>{editing?'✏️ Edit':`➕ ${t('Tambah','Add')}`} Job Family</h2>
          ...
          <div className='flex flex-col gap-3'>
            <div>
              <label className='block text-xs font-semibold text-gray-600 mb-1'>{t('Kode','Code')}</label>
              <input value={form.code} onChange={e=>setForm(f=>({...f,code:e.target.value}))}
                className='w-full px-3 py-2 border border-gray-200 rounded-lg text-sm outline-none focus:border-red-400' />
            </div>
            <div>
              <label className='block text-xs font-semibold text-gray-600 mb-1'>{t('Nama Job Family','Job Family Name')}</label>
              <input value={form.name} onChange={e=>setForm(f=>({...f,name:e.target.value}))}
                className='w-full px-3 py-2 border border-gray-200 rounded-lg text-sm outline-none focus:border-red-400' />
            </div>
            <div>
              <label className='block text-xs font-semibold text-gray-600 mb-1'>{t('Deskripsi','Description')}</label>
        ...
        <div className='lg:col-span-2 bg-white rounded-xl p-6 shadow-sm'>
          ...
          <table className='w-full text-sm'>
            ...
            <tbody>
              {jobFamilies.length ? jobFamilies.map(x=>(
                <tr key={x.id} className='border-t border-gray-100 hover:bg-gray-50'>
                  ...
                  <td className='px-4 py-2.5 text-gray-500 text-xs max-w-xs truncate'>{x.description||'-'}</td>
                  ...
                  <td className='px-4 py-2.5'>
                    <div className='flex gap-2'>
                      ...
                      <button onClick={()=>deleteJobFamily(x.id)} className='px-3 py-1 bg-red-50 text-red-600 text-xs font-semibold rounded-lg hover:bg-red-100'>{t('Hapus','Delete')}</button>
                    </div>
                  </td>
                </tr>
              )) : (
