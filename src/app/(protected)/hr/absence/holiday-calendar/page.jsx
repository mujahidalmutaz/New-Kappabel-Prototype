...
export default function HolidayCalendarPage() {
  ...
  return (
    <div>
      ...
      <p className='text-gray-500 text-sm mb-6'>Kelola hari libur nasional per negara dan tahun.</p>
      ...
      <div className='grid grid-cols-1 lg:grid-cols-3 gap-6'>
        ...
        <div className='bg-white rounded-xl p-6 shadow-sm'>
          <h2 className='text-sm font-bold text-gray-700 mb-4'>
            {editing ? '✏️ Edit Hari Libur' : '➕ Tambah Hari Libur'}
          </h2>
          {msg && (
          ...
          <div className='flex flex-col gap-3'>
            ...
            <div className='flex gap-2 pt-1'>
              ...
              {editing && (
                <button onClick={() => { setEditing(null); setForm({ name:'', date:'', type:'National', recurring:false }) }}
        ...
        <div className='lg:col-span-2 bg-white rounded-xl p-6 shadow-sm'>
          <h2 className='text-sm font-bold text-gray-700 mb-4'>
            🗓️ Daftar Hari Libur — {country} {year}
            <span className='ml-2 text-xs font-normal text-gray-400'>({filtered.length} hari)</span>
          </h2>
          <div className='overflow-x-auto'>
            <table className='w-full text-sm'>
              <thead>
                <tr className='bg-gray-50'>
                  {['Tanggal','Nama','Tipe','Berulang','Aksi'].map(h => (
                ...
                {filtered.length ? filtered.map(h => (
                  <tr key={h.id} className='border-t border-gray-100 hover:bg-gray-50'>
                    ...
                    <td className='px-4 py-2.5'>
                      <span className='text-xs px-2 py-0.5 rounded-full bg-blue-50 text-blue-700 font-semibold'>{h.type}</span>
                    </td>
                  ...
                  <tr>
                    <td colSpan={5} className='px-4 py-8 text-center text-gray-400 text-sm'>
                      Belum ada hari libur untuk {country} {year}.
                    </td>
                  </tr>
                )}
              </tbody>
