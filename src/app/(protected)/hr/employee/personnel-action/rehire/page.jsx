    ...
    <div className='min-h-screen bg-gray-50'>
      <div className={`bg-gradient-to-r ${COLOR} text-white px-8 py-6`}>
        ...
        <div className='grid grid-cols-3 gap-4 mt-4'>
          {[['Total', stats.total, 'bg-white/20'], ['Draft', stats.draft, 'bg-gray-400/30'], ['Applied', stats.applied, 'bg-white/30']].map(([l, v, c]) => (
            <div key={l} className={`${c} rounded-xl px-4 py-3`}><p className='text-xs text-green-100'>{l}</p><p className='text-2xl font-bold'>{v}</p></div>
          ))}
        </div>
      </div>
