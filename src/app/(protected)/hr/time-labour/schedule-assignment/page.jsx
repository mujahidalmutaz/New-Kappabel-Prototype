...
export default function ScheduleAssignmentPage() {
  ...
  const handleSave = () => {
    if (!form.userId || !form.scheduleId || !form.startDate) return flash('Semua field wajib diisi.','error')
  ...
  return (
    <div>
      <h1 className='text-2xl font-bold text-gray-800 mb-1'>Schedule Assignment</h1>
      <p className='text-gray-500 text-sm mb-6'>Assign jadwal kerja ke karyawan.</p>

      <div className='grid grid-cols-1 lg:grid-cols-3 gap-6'>
        <div className='bg-white rounded-xl p-6 shadow-sm'>
          <h2 className='text-sm font-bold text-gray-700 mb-4'>➕ Assign Schedule</h2>
          ...
          <div className='flex flex-col gap-3'>
            <div>
              <label className='block text-xs font-semibold text-gray-600 mb-1'>Karyawan</label>
            ...
            <div>
              <label className='block text-xs font-semibold text-gray-600 mb-1'>Work Schedule</label>
        ...
        <div className='lg:col-span-2 bg-white rounded-xl p-6 shadow-sm'>
          <h2 className='text-sm font-bold text-gray-700 mb-4'>📋 Daftar Assignment</h2>
          <table className='w-full text-sm'>
            <thead>
              <tr className='bg-gray-50'>
                {['Karyawan','Schedule','Mulai','Aksi'].map(h=>(
              ...
              {assignments.length ? assignments.map(a=>(
                <tr key={a.id} className='border-t border-gray-100 hover:bg-gray-50'>
                  ...
                  <td className='px-4 py-2.5'>
                    <button onClick={()=>deleteAssignment(a.id)} className='px-3 py-1 bg-red-50 text-red-600 text-xs font-semibold rounded-lg hover:bg-red-100'>{t('Hapus','Delete')}</button>
                  </td>
                </tr>
              )) : (
                <tr><td colSpan={4} className='px-4 py-8 text-center text-gray-400 text-sm'>Belum ada assignment.</td></tr>
              )}
            </tbody>
          </table>
        </div>
