...
export default function WorkSchedulePage() {
  ...
  return (
    <div>
      <h1 className='text-2xl font-bold text-gray-800 mb-1'>{t('Work Schedule', 'Work Schedule')}</h1>
      <p className='text-gray-500 text-sm mb-6'>{t('Buat jadwal kerja berdasarkan shift pattern.', 'Create work schedules based on shift patterns.')}</p>

      <div className='grid grid-cols-1 lg:grid-cols-3 gap-6'>
        <div className='bg-white rounded-xl p-6 shadow-sm'>
          <h2 className='text-sm font-bold text-gray-700 mb-4'>➕ {t('Buat Schedule', 'Create Schedule')}</h2>
          {msg && (
          ...
          <div className='flex flex-col gap-3'>
            <div>
              <label className='block text-xs font-semibold text-gray-600 mb-1'>{t('Nama Schedule', 'Schedule Name')}</label>
              <input value={form.name} onChange={e => setForm(f => ({ ...f, name: e.target.value }))}
                className='w-full px-3 py-2 border border-gray-200 rounded-lg text-sm outline-none focus:border-red-400' />
            </div>
            <div>
              <label className='block text-xs font-semibold text-gray-600 mb-1'>{t('Shift Pattern', 'Shift Pattern')}</label>
            ...
            <div>
              <label className='block text-xs font-semibold text-gray-600 mb-1'>{t('Tanggal Efektif', 'Effective Date')}</label>
        ...
        <div className='lg:col-span-2 bg-white rounded-xl p-6 shadow-sm'>
          <h2 className='text-sm font-bold text-gray-700 mb-4'>📋 {t('Daftar Work Schedule', 'Work Schedule List')}</h2>
          <div className='overflow-x-auto'>
            <table className='w-full text-sm'>
              <thead>
                <tr className='bg-gray-50'>
                  {[t('Nama','Name'), t('Pattern','Pattern'), t('Efektif Mulai','Effective From'), t('Aksi','Action')].map(h => (
              ...
              <tbody>
                {schedules.length ? schedules.map(s => (
                  ...
                  <tr><td colSpan={4} className='px-4 py-8 text-center text-gray-400 text-sm'>{t('Belum ada schedule.', 'No schedules yet.')}</td></tr>
                )}
              </tbody>
            </table>
          </div>
