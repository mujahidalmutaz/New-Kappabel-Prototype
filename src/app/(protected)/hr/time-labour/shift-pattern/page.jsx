...
export default function ShiftPatternPage() {
  ...
  return (
    <div>
      <h1 className='text-2xl font-bold text-gray-800 mb-1'>{t('Shift Pattern', 'Shift Pattern')}</h1>
      <p className='text-gray-500 text-sm mb-6'>{t('Buat pola shift mingguan untuk karyawan.', 'Create weekly shift patterns for employees.')}</p>

      <div className='grid grid-cols-1 lg:grid-cols-3 gap-6'>
        {/* Form */}
        <div className='bg-white rounded-xl p-6 shadow-sm'>
          <h2 className='text-sm font-bold text-gray-700 mb-4'>➕ {t('Buat Pattern', 'Create Pattern')}</h2>
          {msg && (
          ...
          <div className='flex flex-col gap-3'>
            <div>
              <label className='block text-xs font-semibold text-gray-600 mb-1'>{t('Nama Pattern', 'Pattern Name')}</label>
        ...
        <div className='lg:col-span-2 bg-white rounded-xl p-6 shadow-sm'>
          <h2 className='text-sm font-bold text-gray-700 mb-4'>📋 {t('Daftar Pattern', 'Pattern List')}</h2>
          {patterns.length ? patterns.map(p => (
            ...
            <p className='text-sm text-gray-400'>{t('Belum ada pattern.', 'No patterns yet.')}</p>
          )}
        </div>
      </div>
    </div>
