...
export default function MasterInstructorsPage() {
  ...
  const handleSave = () => {
    ...
    if (editing) {
      ...
      flash('Instructor diperbarui.')
      setEditing(null)
    } else {
      setData(prev => [...prev, { id: Date.now(), ...form }])
      flash('Instructor ditambahkan.')
    }
    setForm(EMPTY)
  }
  ...
  const handleDelete = (id) => { setData(prev => prev.filter(d => d.id !== id)); flash('Instructor dihapus.') }

  return (
    <div>
      <h1 className='text-2xl font-bold text-gray-800 mb-1'>Master Instructors</h1>
      <p className='text-gray-500 text-sm mb-6'>Data trainer/instructor/fasilitator training internal maupun eksternal.</p>
      ...
      <div className='grid grid-cols-1 lg:grid-cols-3 gap-6'>
        <div className='bg-white rounded-xl p-6 shadow-sm'>
          <h2 className='text-sm font-bold text-gray-700 mb-4'>{editing ? '✏️ Edit Instructor' : '➕ Tambah Instructor'}</h2>
        ...
        <div className='lg:col-span-2 bg-white rounded-xl p-6 shadow-sm'>
          <input value={search} onChange={e=>setSearch(e.target.value)} placeholder='Cari instructor...'
            ...
            <table className='w-full text-sm'>
              ...
              <tbody>{filtered.map(d=>(
                <tr key={d.id} className='border-t border-gray-100 hover:bg-gray-50'>
                  ...
                  <td className='px-3 py-2.5 text-gray-500'>{d.experience} thn</td>
