...
export default function MasterContentPage() {
  ...
  return (
    <div>
      ...
      {/* ── Fixed toast ── */}
      {msg && (
        <div className={`fixed bottom-6 right-6 z-50 flex items-center gap-2 px-5 py-3 rounded-xl shadow-xl text-sm font-semibold pointer-events-none transition-all
      ...
      <div className='bg-white rounded-xl p-6 shadow-sm'>
        {/* Header row with search + filter + add button */}
        <div className='flex flex-wrap gap-3 mb-4 items-center'>
          ...
          <div className='flex gap-1 flex-wrap'>
            {['All',...CONTENT_TYPES].map(tp => (
              <button key={tp} onClick={()=>setFilter(tp)}
                className={`px-3 py-1.5 rounded-lg text-xs font-semibold transition ${filter===tp?'bg-red-600 text-white':'bg-gray-100 text-gray-600 hover:bg-gray-200'}`}>{tp}</button>
            ))}
          </div>
          ...
          <div className='bg-white rounded-2xl shadow-2xl w-full max-w-lg max-h-[90vh] overflow-y-auto' onClick={e => e.stopPropagation()}>
            <div className='px-6 py-4 border-b border-gray-100 flex items-center justify-between'>
              <h2 className='text-base font-bold text-gray-800'>
                {editing ? `✏️ ${t('Edit Content','Edit Content')}` : `➕ ${t('Tambah Content','Add Content')}`}
              </h2>
            ...
            <div className='px-6 py-5 flex flex-col gap-3'>
              ...
              <div>
                ...
                <select value={form.type} onChange={e=>setForm(f=>({...f,type:e.target.value,videoSource:null,videoUrl:'',videoFileName:'',videoFileSize:'',videoObjectUrl:''}))}
                  ...
                  {CONTENT_TYPES.map(tp => <option key={tp}>{tp}</option>)}
                </select>
              </div>
            ...
            <div className='px-6 pb-5 flex gap-3'>
              <button onClick={handleSave} className='flex-1 py-2.5 text-sm font-semibold text-white rounded-xl hover:opacity-90 transition' style={{ background: BRAND }}>
                {editing ? t('Simpan Perubahan','Save Changes') : t('Tambah Content','Add Content')}
              </button>
