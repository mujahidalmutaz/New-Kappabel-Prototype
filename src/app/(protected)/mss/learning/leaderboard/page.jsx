...
export default function MssLeaderboardPage() {
  const t = useT()
  const [tab, setTab] = useState('Tim Saya')
  ...
  return (
    <div>
      <h1 className='text-2xl font-bold text-gray-800 mb-1'>Team Leaderboard</h1>
      <p className='text-gray-500 text-sm mb-6'>Peringkat learning anggota tim Anda dan perbandingan antar departemen.</p>

      {/* Team Stats */}
      <div className='p-5 rounded-xl text-white mb-6' style={{background:'linear-gradient(135deg,#8B1A1A,#D7252B)'}}>
        <div className='flex items-center justify-between'>
          <div>
            <div className='text-xs text-white/70'>Peringkat Tim Anda</div>
            <div className='text-3xl font-bold mt-0.5'>#4 dari 12 tim</div>
            <div className='text-sm text-white/80 mt-0.5'>Tim Operations</div>
          </div>
          <div className='grid grid-cols-3 gap-6 text-center'>
            {[['3,735','Total Poin'],['5 orang','Anggota Aktif'],['51%','Compliance Rate']].map(([v,l])=>(
      ...
      <div className='flex gap-2 mb-6'>
        {['Tim Saya', 'Antar Departemen'].map(t=>(
      ...
      {tab === 'Tim Saya' && (
        <div className='space-y-2'>
          {TEAM_BOARD.map(u=>(
            <div key={u.rank} className={`flex items-center gap-4 rounded-xl p-4 border transition hover:shadow-md ${rankBg(u.rank)}`}>
              ...
              <div className='grid grid-cols-5 gap-4 text-center text-xs'>
                {[[u.points,'Poin 🏅'],[u.badges,'Badge 🏆'],[u.completed,'Selesai 📚'],[u.cpd,'CPD ⭐'],[u.streak+'d','Streak 🔥']].map(([v,l])=>(
          ...
          <div className='mt-4 bg-blue-50 border border-blue-200 rounded-xl p-4'>
            <h3 className='font-semibold text-blue-700 mb-2 text-sm'>💡 Insight Tim</h3>
            <ul className='text-xs text-blue-600 space-y-1'>
              <li>• <b>Dewi Sari</b> adalah top learner tim dengan 1.250 poin dan 45 hari streak!</li>
              <li>• <b>Budi Rahayu & Rizky Pratama</b> perlu dorongan — progress masih jauh di bawah rata-rata tim.</li>
              <li>• Rata-rata CPD tim: <b>62 points</b> — di bawah target 80 pts per kuartal.</li>
            </ul>
          </div>
        </div>
      )}

      {tab === 'Antar Departemen' && (
        <div>
              ...
              <div key={d.rank} className={`flex items-center gap-4 rounded-xl p-4 border transition ${d.isMyTeam?'border-red-300 bg-red-50':rankBg(d.rank)}`}>
                ...
                <div className='flex-1'>
                  <div className='flex items-center gap-2'>
                    ...
                    {d.isMyTeam && <span className='text-xs bg-red-100 text-red-700 px-2 py-0.5 rounded-full font-semibold'>Tim Anda</span>}
                  </div>
                  <div className='text-xs text-gray-400'>{d.dept} · {d.members} anggota</div>
                </div>
                <div className='text-right'>
                  <div className='font-bold text-gray-800'>{d.points.toLocaleString('id-ID')} pts</div>
                  <div className='text-xs text-gray-400'>{(d.points/d.members).toFixed(0)} pts/orang</div>
                </div>
                <div className='w-32'>
          ...
          <div className='bg-yellow-50 border border-yellow-200 rounded-xl p-4 text-sm text-yellow-700'>
            💡 Tim Anda berada di peringkat <b>#4</b>. Perlu <b>{(COMPANY_BOARD[2].points - COMPANY_BOARD[3].points).toLocaleString('id-ID')} poin lagi</b> untuk naik ke peringkat 3.
          </div>
        </div>
      )}
    </div>
