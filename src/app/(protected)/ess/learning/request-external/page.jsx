...
export default function RequestExternalTrainingPage() {
  ...
  const handleSubmit = () => {
    ...
    flash(t('Pengajuan training eksternal berhasil disimpan sebagai Draft.','External training request saved as Draft.'))
    setForm(EMPTY)
    setStep('list')
  }
  ...
  return (
    <div>
      ...
      <p className='text-gray-500 text-sm mb-6'>{t('Ajukan permintaan mengikuti training/sertifikasi eksternal di luar program internal perusahaan.','Submit a request to attend external training/certification outside the company\'s internal program.')}</p>
              ...
              <div key={r.id} className='bg-white rounded-xl p-5 shadow-sm border border-gray-100'>
                ...
                <div className='flex items-center justify-between'>
                  ...
                  <div className='flex gap-2 text-xs text-gray-400'>
                    <span>{t('Diajukan:','Submitted:')} {r.submitted}</span>
                  </div>
                </div>
              </div>
            ))}
      ...
      {step==='form' && (
        <div className='bg-white rounded-xl p-6 shadow-sm max-w-2xl'>
          ...
          <div className='flex gap-3 mt-6'>
            ...
            <button onClick={()=>{const d={...form};setRequests(prev=>[...prev,{id:Date.now(),...d,cost:Number(d.cost)||0,status:'Draft',submitted:new Date().toISOString().slice(0,10)}]);setForm(EMPTY);setStep('list');flash(t('Disimpan sebagai Draft.','Saved as Draft.'))}}
